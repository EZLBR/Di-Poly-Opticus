...
export default function CreatorStudio({ setView, onOpenDesigns }) {
  ...
  function updateAISuggestions(landmarks, eyeDistance) {
    ...
    if (model === "round") {
      list.push(language === "pt" 
        ? "Armações Redondas suavizam traços marcados e mandíbulas quadradas."
        : "Round frames beautifully soften strong lines and square jaw structures.");
    } else if (model === "square") {
      list.push(language === "pt" 
        ? "Armações Quadradas adicionam definição e harmonia a rostos arredondados."
        : "Square frames add outstanding definition to rounder, softer facial features.");
    } else {
      list.push(language === "pt" 
        ? "A forma Hexagonal equilibra rostos ovais com uma expressão futurista e arrojada."
        : "The Hexagonal shape balances oval face shapes with a futuristic and bold mood.");
    }

    if (topBar) {
      list.push(language === "pt" 
        ? "A barra dupla (Top Bar) adiciona estilo aviador clássico, ideal para pontes nasais baixas."
                  ...
                  {!faceDetected && (
                    ...
                    >
                      ...
                      <p style={{ margin: 0, fontSize: "13px", fontWeight: "600" }}>
                        {language === "pt"
                          ? "Procurando rosto... Por favor posicione-se em frente à câmera."
